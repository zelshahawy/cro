...
History get_history(sqlite3 *db, const std::string &name) {
  ...
  {
    const char *sql =
        "SELECT default_increment_lb FROM exercises WHERE name = ?1 LIMIT 1;";
  ...
  {
    const char *sql = "SELECT s.weight_lb FROM sets s "
...
int log_set(sqlite3 *db, const std::string &name, int reps, double weight_lb,
  ...
  {
    const char *sql =
        "INSERT INTO workouts(started_at, notes) VALUES (?1, ?2);";
    sqlite3_stmt *st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
  ...
  int set_index = 1;
  {
    const char *sql = "SELECT COALESCE(MAX(set_index)+1,1) FROM sets WHERE "
                      "workout_id=?1 AND exercise=?2;";
    sqlite3_stmt *st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
  ...
  int set_id = 0;
  {
    const char *sql = "INSERT INTO sets(workout_id, exercise, set_index, reps, "
                      "weight_lb, rir) VALUES (?1, ?2, ?3, ?4, ?5, ?6);";
    sqlite3_stmt *st = nullptr;
    if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
...
std::vector<std::string> all_exercises(sqlite3 *db) {
  ...
  while (sqlite3_step(st) == SQLITE_ROW) {
    ...
    out.emplace_back(reinterpret_cast<const char *>(
        txt ? txt : reinterpret_cast<const unsigned char *>("")));
  }
  sqlite3_finalize(st);
  return out;
}
  ...
  try {
    ...
    if (headers.find("date") == headers.end() ||
      ...
      result.errors.push_back(
          "Missing required columns: Date, Exercise, Reps, Weight");
      return result;
    }
    ...
    auto notes_col = headers.find("notes") != headers.end()
                         ? static_cast<size_t>(headers["notes"])
                         : SIZE_MAX;

    // Group by workout (same date = same workout)
    std::map<std::string,
             std::vector<std::tuple<std::string, int, double, std::string>>>
        workouts;
    ...
    for (; row_iter != ws.rows().end(); ++row_iter) {
      try {
        ...
        if (cells.size() <=
            std::max({date_col, exercise_col, reps_col, weight_col}))
        ...
        try {
          ...
          result.errors.push_back("Invalid reps value in row with exercise: " +
                                  exercise);
          continue;
        }
        ...
        try {
          ...
          result.errors.push_back(
              "Invalid weight value in row with exercise: " + exercise);
          continue;
        }
        ...
        result.errors.push_back("Error processing row: " +
                                std::string(e.what()));
      }
    }
      ...
      try {
        ...
        for (const auto &[exercise, reps, weight, notes] : sets) {
          try {
            ...
            {
              const char *sql = "INSERT OR IGNORE INTO exercises(name, "
                                "default_increment_lb) VALUES (?1, 2.5);";
              sqlite3_stmt *st = nullptr;
              if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
            ...
            {
              const char *sql =
                  "INSERT INTO sets(workout_id, exercise, set_index, reps, "
                  "weight_lb, rir) VALUES (?1, ?2, ?3, ?4, ?5, ?6);";
              sqlite3_stmt *st = nullptr;
              if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
                throw std::runtime_error("prepare insert set failed");
            ...
            result.errors.push_back("Error inserting set for " + exercise +
                                    ": " + e.what());
          }
        }
        ...
        result.errors.push_back("Error processing workout for date " + date +
                                ": " + e.what());
      }
    }
  } catch (const std::exception &e) {
    result.errors.push_back("Failed to read Excel file: " +
                            std::string(e.what()));
  }
